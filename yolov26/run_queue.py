#!/usr/bin/env python3
"""
YOLOv26 Training Queue Runner - Execute Training Queue
...
def update_task_status(queue_data, idx, status, error=None):
    queue_data["tasks"][idx]["status"] = status
    if error:
        queue_data["tasks"][idx]["error"] = str(error)
    if status == "completed":
...
def train_task(task):
    model_source = task.get("model_source", f"{task['model']}.yaml")
    pretrained   = task.get("pretrained", False)
    log(f"Initializing {task['model']} from: {model_source}")
    ...
    best_epoch   = 0
    try:
        results_csv = Path(task["project"]) / task["name"] / "results.csv"
    ...
    log_sep()
    log(f"📊 Best Epoch: {best_epoch}  Fitness: {best_fitness:.4f}")
    log_sep()
...
def print_summary(queue_data):
    ...
    log_sep()
    log(f"Total: {queue_data['total_tasks']}  "
        f"Completed: {queue_data.get('completed_tasks', 0)}  "
        f"Failed: {queue_data.get('failed_tasks', 0)}")
    log_sep()
...
def main():
    log_sep()
    log("YOLOv26 Training Queue Runner Starting")
    log_sep()

    queue_data = load_queue()
    if queue_data is None:
        log(f"❌ No queue file found at {QUEUE_FILE}")
        log("   Run 'python setup_queue.py' first.")
        sys.exit(1)

    if queue_data["status"] == "completed":
        log("⚠️  Queue already completed.")
        sys.exit(0)
    ...
    for i, task in enumerate(queue_data["tasks"]):
        ...
        log(f"  Init:       {task.get('model_source', 'yaml')}")
        ...
        try:
            ...
            log(f"✅ Task #{task_num} COMPLETED in {duration}  (best epoch {best_epoch})")
