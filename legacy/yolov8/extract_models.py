#!/usr/bin/env python3
"""
Extract and Organize Trained YOLOv8 Models
...
def clean_dataset_name(data_path):
    """Derive a clean short name from the data.yaml path."""
    try:
...
def get_model_size(model_str):
    for s in ['n', 's', 'm', 'l', 'x']:
        if f'yolov8{s}' in str(model_str).lower():
            return s
    return 'unknown'
    ...
    for dataset, group in by_dataset.items():
        ...
        for meta in sorted(group, key=lambda x: x['best_fitness'], reverse=True):
            ...
            try:
                ...
                print(f"   Model:   yolov8{meta['model_size']} @ {meta['imgsz']}px")
...
def main():
    parser = argparse.ArgumentParser(description='Extract organized YOLOv8 models')
